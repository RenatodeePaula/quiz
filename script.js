...
function loadQuestion() {
  ...
  choiceElements.forEach( (elem, indice) => {
    elem.innerText = choices[indice]
  });
  answerChosen = false;
};
...
function checkAnswer(ev) {
  ...
  if(ev.target.innerText === questions[currentQuestion].answer) {
    score++;
...
nextButton.addEventListener('click', () => {
  ...
  if(currentQuestion < questions.length) {
    ...
    alert(`Fim de Jogo! Você acertoou ${score} de ${questions.length}.`)
    restartQuiz();
  };
});
